...
Для запуска MLflow сервера используйте команду:
...
1. Автоматически создаст необходимые директории:
   - `./mlruns` - для хранения метаданных экспериментов
   - `./artifacts` - для хранения артефактов (модели, метрики, графики)
2. Запустит MLflow сервер на http://127.0.0.1:8080
3. Настроит:
   - Backend store в директории `./mlruns`
   - Artifact store в директории `./artifacts`

Важные замечания:

- Оставьте терминал с запущенным сервером открытым
- Для остановки сервера используйте Ctrl+C
- Директория `artifacts` должна быть под контролем DVC для версионирования
  моделей и артефактов
- Директорию `mlruns` не нужно добавлять под контроль версий, так как она
  содержит только метаданные

После запуска сервера вы можете открыть веб-интерфейс MLflow по адресу
http://127.0.0.1:8080 и отслеживать там все эксперименты, метрики и артефакты.
