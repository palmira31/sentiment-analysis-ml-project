...
class RNNClassifier(pl.LightningModule):
    def __init__(self, input_dim, hidden_dim=128, output_dim=3, lr=1e-3, num_layers=1, bidirectional=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        final_feature = hn[-1] if not self.hparams.bidirectional else torch.cat((hn[-2], hn[-1]), dim=1)
